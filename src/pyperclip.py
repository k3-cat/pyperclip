...
def init_cb_clipboard():
    def copy_cb(text: str):
        text = str(text)  # Converts non-str values to str.
        p = subprocess.Popen(["cb", "--no-confirmation"], stdin=subprocess.PIPE, close_fds=True)
        p.communicate(input=text.encode(ENCODING))

    def paste_cb():
        p = subprocess.Popen(["cb", "--no-confirmation", "paste"], stdout=subprocess.PIPE, close_fds=True)
        stdout, stderr = p.communicate()
        return stdout.decode(ENCODING)

    return copy_cb, paste_cb


def init_osx_pbcopy_clipboard():
    def copy_osx_pbcopy(text: str):
        text = str(text)  # Converts non-str values to str.
...
def determine_clipboard():
    ...
    if _executable_exists("cb"):
        return init_cb_clipboard()
...
def lazy_load_stub_copy(text: str):
    """
